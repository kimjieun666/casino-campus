...
public class Deck {
    ...
    // 
    // 구현 힌트:
    // 1. 필드 선언과 동시에 초기화하는 방법을 사용하세요
    // 2. ArrayList를 생성하면서 바로 카드를 추가할 수 있습니다
    // 3. 이중 반복문을 사용하여 4개 무늬 × 13개 랭크 = 52장을 생성하세요
    // 4. 첫 번째 반복문: Suit.values()로 모든 무늬를 순회
    // 5. 두 번째 반복문: Rank.values()로 모든 랭크를 순회
    // 
    // 테스트 실패 시 확인사항:
    // - "덱은 52장의 카드를 가져야 합니다" 에러: 카드 생성 로직이 잘못되었습니다
    // - "빈 컬렉션입니다" 경고: 카드를 추가하는 코드가 실행되지 않았습니다
    ...
    public void shuffle() {
        ...
        // 
        // 구현 힌트:
        // 1. java.util.Collections 클래스를 살펴보세요
        // 2. 리스트의 요소들을 무작위로 섞는 메서드가 있습니다
        // 3. cards 필드를 인자로 전달하면 됩니다
        // 
        // 테스트 실패 시 확인사항:
        // - "카드 순서가 변경되지 않았습니다" 에러: shuffle 메서드를 호출하지 않았습니다
        // - "카드 수가 변경되었습니다" 에러: 섞기 과정에서 카드를 추가하거나 제거했습니다
